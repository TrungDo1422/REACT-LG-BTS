...
const BannerForm = ({ setLoading, setOnEdit, id, title, content }) => {
    const initState = {
        title: title,
        content: content,
    };
    ...
    const onSubmit = async (event) => {
        ...
        const res = await bannerApi.updateBanner({ id, title: titleInput, content: contentInput });
        setBannerData({
            ...bannerData,
            res,
        });
    ...
    return (
        <div className="bg-lightblue">
            <form className="mt-4 form-group" onSubmit={onSubmit} >
                ...
                <button className="btn btn-warning col-6" type="submit" onClick={() => setOnEdit(false)}>
                    <HighlightOffSharp />
                </button>
            </form>
        </div>
